...
int main()
{
    ...
    if (row == col)
    {
        ...
        if (deter == 0)
            printf("\n Inverse of Matrix is not possible because the determinant is 0!!\n");
        else
            cofac(matrix, row,&result[0]);
    }
    else
        printf("\n Inverse of Matrix is possible for Square matrix Please input the same number of rows and columns \n");
    return (0);
}
